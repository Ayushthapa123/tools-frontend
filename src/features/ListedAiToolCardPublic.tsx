      ...
      <div className="p-3 pt-2 space-y-3 ">
        ...
        <div className="grid grid-cols-1 gap-3">
          ...
          {tool?.domains && tool.domains.length > 0 && (
            <div className="space-y-1.5">
              ...
              <div className="flex flex-wrap gap-1.5">
                {tool.domains.slice(0, expandedSections.domains ? undefined : 4).map((domain, index) => (
                ...
                {tool.domains.length > 4 && (
                  <button
                    ...
                    className="bg-gray-100 hover:bg-gray-200 text-gray-600 text-xs px-2 py-0.5 rounded-full font-medium cursor-pointer transition-colors duration-200"
                  >
                    {expandedSections.domains ? 'Less' : `+${tool.domains.length - 4}`}
                  </button>
                )}
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 gap-3">
          ...
          {tool?.useCases && tool.useCases.length > 0 && (
            <div className="space-y-1.5">
              ...
              <div className="flex flex-wrap gap-1.5">
                {tool.useCases.slice(0, expandedSections.useCases ? undefined : 3).map((useCase, index) => (
                ...
                {tool.useCases.length > 3 && (
                  <button
                    ...
                    className="bg-gray-100 hover:bg-gray-200 text-gray-600 text-xs px-2 py-0.5 rounded-full font-medium cursor-pointer transition-colors duration-200"
                  >
                    {expandedSections.useCases ? 'Less' : `+${tool.useCases.length - 3}`}
                  </button>
                )}
              </div>
            </div>
          ...
          {tool?.toolUserTypes && tool.toolUserTypes.length > 0 && (
            <div className="space-y-1.5">
              ...
              <div className="flex flex-wrap gap-1.5">
                {tool.toolUserTypes.slice(0, expandedSections.userTypes ? undefined : 3).map((userType, index) => (
                ...
                {tool.toolUserTypes.length > 3 && (
                  <button
                    ...
                    className="bg-gray-100 hover:bg-gray-200 text-gray-600 text-xs px-2 py-0.5 rounded-full font-medium cursor-pointer transition-colors duration-200"
                  >
                    {expandedSections.userTypes ? 'Less' : `+${tool.toolUserTypes.length - 3}`}
                  </button>
                )}
              </div>
            </div>
        ...
        {tool?.aiCapabilities && tool.aiCapabilities.length > 0 && (
          <div className="space-y-1.5">
            ...
            <div className="flex flex-wrap gap-1.5">
              {tool.aiCapabilities.slice(0, expandedSections.aiCapabilities ? undefined : 4).map((capability, index) => (
              ...
              {tool.aiCapabilities.length > 4 && (
                <button
                  ...
                  className="bg-gray-100 hover:bg-gray-200 text-gray-600 text-xs px-2 py-0.5 rounded-full font-medium cursor-pointer transition-colors duration-200"
                >
                  {expandedSections.aiCapabilities ? 'Less' : `+${tool.aiCapabilities.length - 4}`}
                </button>
              )}
            </div>
          </div>
        ...
        {/* Collapsible Content */}
        <div className={`transition-all duration-300 grid gap-3 ease-in-out overflow-hidden ${isExpanded ? 'max-h-[1500px]' : 'max-h-0'}`}>
          <div className="space-y-1.5">
            ...
            {tool?.modalities && tool.modalities.length > 0 && (
              <div className="space-y-1.5">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {tool.modalities.slice(0, expandedSections.modalities ? undefined : 3).map((modality, index) => (
                  ...
                  {tool.modalities.length > 3 && (
                    <button
                      ...
                      className="bg-gray-100 hover:bg-gray-200 text-gray-600 text-xs px-2 py-0.5 rounded-full font-medium cursor-pointer transition-colors duration-200"
                    >
                      {expandedSections.modalities ? 'Less' : `+${tool.modalities.length - 3}`}
                    </button>
                  )}
                </div>
              </div>
            ...
            {tool?.integrationOptions && tool.integrationOptions.length > 0 && (
              <div className="space-y-1.5">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {tool.integrationOptions.slice(0, expandedSections.integrations ? undefined : 6).map((integration, index) => (
                  ...
                  {tool.integrationOptions.length > 6 && (
                    <button
                      ...
                      className="bg-gray-100 hover:bg-gray-200 text-gray-600 text-xs px-2 py-0.5 rounded-full font-medium cursor-pointer transition-colors duration-200"
                    >
                      {expandedSections.integrations ? 'Less' : `+${tool.integrationOptions.length - 6}`}
                    </button>
                  )}
                </div>
              </div>
          ...
          <div className="space-y-1.5"> 
            ...
            {tool?.pricingType && tool.pricingType.length > 0 && (
              <div className="space-y-1.5">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {tool.pricingType.slice(0, expandedSections.pricing ? undefined : 3).map((price, index) => (
                  ...
                  {tool.pricingType.length > 3 && (
                    <button
                      ...
                      className="bg-gray-100 hover:bg-gray-200 text-gray-600 text-xs px-2 py-0.5 rounded-full font-medium cursor-pointer transition-colors duration-200"
                    >
                      {expandedSections.pricing ? 'Less' : `+${tool.pricingType.length - 3}`}
                    </button>
                  )}
                </div>
              </div>
      ...
      <div className="bg-gray-50 px-3 py-2 border-t border-gray-100 justify-end">
        <div className="flex items-center justify-between ">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs px-2.5 py-1.5 rounded-md font-medium transition-colors duration-200"
          >
            {isExpanded ? (
              <>
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {isExpanded && (
          <div className="flex gap-3 text-xs text-gray-500 mt-2 pt-2 border-t border-gray-200">
