...
export const PlaceBlogs = ({ setActiveRoomCount ,isviewonly=false}: { setActiveRoomCount?: (count: number) => void ,isviewonly?:boolean}) => {
  ...
  const fetchData = async () => {
    const res = await queryBlogPosts({blogTags: [BlogTags.TopTenPlaces]});
    ...
    if (deleteBlogPost) {
      mutateAsync({ id: Number(deletedBlogPostId) }).then(res => {
        if (res?.deleteBlogPost) {
          ...
          queryClient.invalidateQueries({ queryKey: ['getBlogPlacePosts'] });
        } else {
