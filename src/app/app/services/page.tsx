...
export default function ServicesPage() {
  ...
  const createService = useGraphqlClientRequest<
    CreateServiceMutation,
    CreateServiceMutationVariables
  ...
  const updateService = useGraphqlClientRequest<
    UpdateServiceMutation,
    UpdateServiceMutationVariables
  ...
  return (
    <div className="mx-auto mt-1 rounded-xl border border-gray-100 bg-white p-6 shadow-lg">
      <h2 className="mb-4 text-2xl font-bold text-primary">Select Services</h2>
      <div className="mb-6 grid grid-cols-2 items-center gap-4 md:grid-cols-3 lg:grid-cols-4">
        {hServices?.map(service => (
          <label
            key={service.id}
            className={`hover:bg-blue-50 flex cursor-pointer items-start gap-3 rounded-lg border border-gray-200 p-3 transition-colors ${
              selectedServices.includes(service.name) ? 'bg-blue-100 border-blue-400' : 'bg-white'
            }`}
          >
              ...
              <div>
                <span
                  className={`text-gray-700 ${
                    selectedServices.includes(service.name) ? 'font-semibold text-primary' : ''
                  }`}
                >
                  {service.name}
                </span>
