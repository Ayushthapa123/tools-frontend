                ...
                <div className="mt-2 grid grid-cols-2 gap-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start">
                      <svg className="mr-1.5 mt-0.5 h-4 w-4 flex-shrink-0 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
                      </svg>
                      <div className="flex flex-col">
                        <span className="text-sm font-medium text-gray-700">{booking.room.name}</span>
                        <span className="text-xs text-gray-500">Room #{booking.room.roomNumber}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start">
                      ...
                      <div className="flex flex-col">
                        <span className="text-xs text-gray-500">
                          Check-in: {eventInfo.event.start ? new Date(eventInfo.event.start).toLocaleDateString() : 'N/A'}
                        </span>
                        <span className="text-xs text-gray-500">
                          Check-out: {eventInfo.event.end ? new Date(eventInfo.event.end).toLocaleDateString() : 'N/A'}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start">
                      ...
                      <div className="flex flex-col">
                        <span className="text-xs text-gray-500">Payment Status: {booking.status || 'N/A'}</span>
                        <span className="text-xs text-gray-500">Amount: {booking.room?.price?.amount || 'N/A'} {booking.room?.price?.currency || ''}</span>
                      </div>
                    </div>
                  </div>
                </div>
