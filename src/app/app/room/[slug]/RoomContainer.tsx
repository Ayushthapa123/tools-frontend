...
export default function RoomContainer({ params }: { params: { slug: string } }) {
  ...
  return (
    <Suspense>
      <div>{!isLoading && <RoomForm params={params} room={room as Room | undefined} />}</div>
    </Suspense>
  );
}
...
function RoomForm({ params, room }: { params: { slug: string }; room: Room | undefined | null }) {
  ...
  const mutateCreateRoom = useGraphqlClientRequest<CreateRoomMutation, CreateRoomMutationVariables>(
    CreateRoom.loc?.source.body!,
  );

  const { mutateAsync } = useMutation({ mutationFn: mutateCreateRoom });

  const mutateUpdateRoom = useGraphqlClientRequest<UpdateRoomMutation, UpdateRoomMutationVariables>(
      ...
      if (!isEdit) {
        mutateAsync({ createRoomInput: input }).then(res => {
          if (res?.createRoom?.data?.id) {
            ...
            router.push(`/app/room/${res?.createRoom?.data?.id}`);
            setCurrentStep(2);
          } else {
        ...
        mutateUpdateRoomAsync({ updateRoomInput: { ...input, id: Number(room?.data?.id) } }).then(
          res => {
  ...
  return (
    <div className="w-full">
      <div className="rounded-lg bg-white p-6 shadow">
        ...
        {currentStep === 2 && (
          <SetPriceForm
            price={room?.data?.price as PriceData | undefined}
            roomId={Number(room?.data?.id)}
            onNext={handleNext}
        ...
        {currentStep === 3 && (
          <UploadPhotos
            ...
            roomImages={room?.data?.image as RoomImage[]}
            roomId={Number(room?.data?.id)}
          />
        )}
        {currentStep === 4 && (
          <RoomAmenity handleBack={handleBack} roomId={Number(room?.data?.id)} />
        )}
      </div>
    </div>
  );
