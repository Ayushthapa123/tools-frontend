...
import { TravelDestinationFinderInput } from 'src/gql/graphql';
export default function Parameters() {
  const { control, formState: { errors } } = useFormContext<TravelDestinationFinderInput>();
...
const travelCompanionTypeOptions = [
  ...
  return (
    ...
    <div className="">
        <TextInput
          name="travelBudgetUSD"
          type="number"
          placeholder="$"
          control={control}
          label="Expected Travel Budget(USD)"
          // required
          helpertext={errors?.travelBudgetUSD && 'travelBudgetUSD' in errors.travelBudgetUSD && (errors.travelBudgetUSD as any)?.type === 'required' ? 'Number of Days is required' : ''}
          error={!!(errors?.travelBudgetUSD && 'travelBudgetUSD' in errors.travelBudgetUSD && (errors.travelBudgetUSD as any))}
        />
      </div>
      <div className="">
        <TextInput
