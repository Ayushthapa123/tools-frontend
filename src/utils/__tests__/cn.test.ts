...
describe('cn utility function', () => {
  ...
  test('handles Tailwind utility conflicts', () => {
    ...
    expect(cn('custom-class bg-red-500', 'bg-blue-500')).toBe('custom-class bg-blue-500');

    // Test with multiple conflicts and non-conflicts
    expect(cn('bg-red-500 text-sm custom-1', 'bg-blue-500 text-lg custom-2')).toBe(
      'custom-1 custom-2 bg-blue-500 text-lg',
    );
  });

  test('handles complex nested arrays and objects', () => {
    expect(
      cn('class1', ['class2', ['class3', { class4: true }]], { class5: true, class6: false }),
    ).toBe('class1 class2 class3 class4 class5');
  });

  test('handles real-world Tailwind examples', () => {
    expect(
      cn('flex items-center', 'p-4 sm:p-6 lg:p-8', {
        'bg-gray-100': true,
